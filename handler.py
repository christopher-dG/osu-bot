...
def handler(event, _):
    ...
    try:
        if post.saved:
            ...
            return resp
    except Exception as e:  # Quite likely doesn't exist.n
        print(e)
        resp["statusCode"] = 400
        body["error"] = str(e)
        resp["body"] = json.dumps(body)
    ...
    if any(c.author.name == osubot.consts.reddit_user for c in post.comments):
        print("Post already has a reply")
        # Don't return here, we'll re-save and upvote just in case.
    elif os.environ.get("LIVE", "false").lower() == "true":
        c = post.reply(reply)
